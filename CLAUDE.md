...
Dựng bản mới: tạo dự án Supabase → chạy `001_init.sql` … `010_sound_bank.sql` (theo thứ tự, tất cả trong `supabase/migrations/`) → đăng ký 1
...
- **SQL + RLS chéo vai trò:** `npm i --no-save @electric-sql/pglite` rồi `node supabase/tests/rls.test.mjs` (133 kiểm tra) và
