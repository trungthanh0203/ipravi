...
Dựng bản mới: tạo dự án Supabase → chạy `001_init.sql` … `015_title_tr.sql` (theo thứ tự, tất cả trong `supabase/migrations/`) → đăng ký 1
...
- **SQL + RLS chéo vai trò:** `npm i --no-save @electric-sql/pglite` rồi `node supabase/tests/rls.test.mjs` (155 kiểm tra) và
