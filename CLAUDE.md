...
Dựng bản mới: tạo dự án Supabase → chạy `001_init.sql` … `017_skill_stats.sql` (theo thứ tự, tất cả trong `supabase/migrations/`) → đăng ký 1
...
- **Hàm thuần + Worker + CSV + TTS:** `node tests/unit.test.mjs`, `node tests/admin.test.mjs` (120 kiểm tra), `node tests/practice.test.mjs` (Luyện tập + huy hiệu, 87 kiểm tra), `node tests/autofill.test.mjs` (Thêm nhanh, 39 kiểm tra) và `node tests/curriculum.test.mjs` (CSV giáo trình) — không cần cài gì.
- **SQL + RLS chéo vai trò:** `npm i --no-save @electric-sql/pglite` rồi `node supabase/tests/rls.test.mjs` (179 kiểm tra) và
