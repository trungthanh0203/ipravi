...
- **Diễn giải bài học** (`lessons.description`, text thuần tiếng Việt — KHÔNG phải jsonb đa ngôn ngữ như `title_tr`,
  migration 020): hiện ngay dưới tên bài ở màn "Bắt đầu" (`child/lesson.js`, class `.lesson-desc`) — bài chưa có thì
  không hiện dòng này, không lỗi. **Chưa có UI admin để sửa** (chưa làm CSV/quickadd cho cột này) — muốn đổi thì sửa
  thẳng bằng SQL. Nội dung cho giáo trình hiện có (137 bài): `supabase/seed/003_lesson_descriptions.sql` (chạy TAY sau
  020; chỉ bổ sung — bài nào `description` đã có giá trị thì giữ nguyên, không ghi đè; thêm bài mới vào CSV thì phải
...
- **SQL + RLS chéo vai trò:** `npm i --no-save @electric-sql/pglite` rồi `node supabase/tests/rls.test.mjs` (183 kiểm tra) và
  `node supabase/tests/seed.test.mjs` (13). Chạy MỌI migration theo thứ tự trên Postgres trong bộ nhớ, giả lập auth/role của Supabase
