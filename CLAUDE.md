...
Dựng bản mới: tạo dự án Supabase → chạy `001_init.sql` … `013_trace.sql` (theo thứ tự, tất cả trong `supabase/migrations/`) → đăng ký 1
...
- **SQL + RLS chéo vai trò:** `npm i --no-save @electric-sql/pglite` rồi `node supabase/tests/rls.test.mjs` (150 kiểm tra) và
