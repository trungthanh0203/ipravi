...
- **Hàm thuần + Worker + CSV + TTS:** `node tests/unit.test.mjs`, `node tests/admin.test.mjs` (120 kiểm tra), `node tests/practice.test.mjs` (Luyện tập + huy hiệu, 87 kiểm tra), `node tests/autofill.test.mjs` (Thêm nhanh, 43 kiểm tra) và `node tests/curriculum.test.mjs` (CSV giáo trình) — không cần cài gì.
