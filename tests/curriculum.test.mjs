...
== ${f}: ${v.items.length} mục, ${v.errors.length} lỗi, ${v.warnings.length} cảnh báo`);
  ...
  const letterItems = v.items.filter((i) => i.type === "letter" && i.unit === "Chữ cái");
  ...
  // Chữ hoa: đủ 29 chữ cái + 11 chữ ghép, mỗi cặp đúng dạng "A a" (chữ thường = chữ hoa viết thường), có âm đọc
  const caps = v.items.filter((i) => i.unit === "Chữ hoa" && i.type === "letter");
  const upperSet = caps.map((i) => caseParts(i.vi)?.upper);
  for (const i of caps) { if (!caseParts(i.vi)) fail(`chữ hoa "${i.vi}" không đúng dạng "Chữ-hoa chữ-thường"`); if (!i.say) fail(`chữ hoa "${i.vi}" chưa có say`); }
  const need29 = [..."AĂÂBCDĐEÊGHIKLMNOÔƠPQRSTUƯVXY"], digraphs = ["Ch", "Gh", "Gi", "Kh", "Ng", "Ngh", "Nh", "Ph", "Qu", "Th", "Tr"];
  const miss = [...need29, ...digraphs].filter((u) => !upperSet.includes(u));
  if (miss.length || upperSet.length !== new Set(upperSet).size) fail("chữ hoa thiếu/trùng: " + miss.join(" "));
  // âm đọc của chữ hoa phải khớp chữ thường ở chủ đề "Chữ cái"
  const lowerSay = new Map(letterItems.map((i) => [i.vi, i.say]));
  for (const i of caps) { const p = caseParts(i.vi); if (p && p.lower.length === 1 && lowerSay.has(p.lower) && lowerSay.get(p.lower) !== i.say) fail(`chữ hoa "${i.vi}": say "${i.say}" khác chữ thường "${lowerSay.get(p.lower)}"`); }
